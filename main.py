...
async def help_command(update: Update, context) -> None:
    ...
    help_text = (
        ...
        "👥 ИГРАТЬ С ДРУЗЬЯМИ - игра с друзьями"
    )
...
async def handle_rules(update: Update, context) -> None:
    ...
    rules_text = (
        "📖 ПРАВИЛА ИГРЫ\n\n"
        "🎯 Суть:\n"
        f"10 участников играют {config.config.ROUNDS_PER_GAME} раундов по "
        f"{config.config.QUESTIONS_PER_ROUND} вопросов.\n\n"
        "📉 Выбывание:\n"
        ...
        "🏆 Победитель:\n"
        "Побеждает тот, кто остаётся последним из 10."
    )
