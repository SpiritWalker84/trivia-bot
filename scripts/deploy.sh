...
try:
    ...
    if 'shuffled_options' in content:
        ...
        pattern = r"(# Create bot AI\s+bot_ai = BotAI\(difficulty\)\s+)# Generate answer\s+options = \[\]\s+if question\.option_a:\s+options\.append\('A'\)\s+if question\.option_b:\s+options\.append\('B'\)\s+if question\.option_c:\s+options\.append\('C'\)\s+if question\.option_d:\s+options\.append\('D'\)\s+bot_answer = bot_ai\.generate_answer\(\s+question\.id,\s+question\.correct_option,\s+options\s+\)"
        
        replacement = r"""\1# Get available options (after shuffling)
            options = []
        ...
        if new_content != content:
            ...
            if old_text in content:
                ...
                print("Could not find exact pattern to replace. Manual update may be needed.")
